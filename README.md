...
In the first 10 seconds before latency test, PTP events are exchanged per second for calculating **time offset** between server and clients.

During the latency test, there is no PTP event anymore. To avoid the negative latency, the **time offset** is dynamically alleviated to make latency positive.
